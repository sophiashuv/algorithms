...
public:
    ...
    Matrix operator *(const Matrix &matrix) {
        if (matrix.y_length == x_length && matrix.x_length == y_length) {
            ...
            for (size_t i = 0; i < this->matrix.size(); i++)
                for (size_t j = 0; j < matrix.matrix.size(); j++)
                    if (this->matrix[i].y == matrix.matrix[j].x) {
                        ...
                        if (temp_value != 0) temp_vector.push_back(Node<T>(temp_value, this->matrix[i].x, matrix.matrix[j].y));
                    }
            ...
            for (size_t i = 0; i < temp_vector.size(); i++) if (temp_vector[i].x < x_length) result.matrix.push_back(temp_vector[i]);
            return result;
        } else throw "Matrix size exception!";
    }

    T check(size_t i, size_t j) {
        for (size_t k = 0; k < matrix.size(); k++) if (matrix[k].x == i && matrix[k].y == j) return matrix[k].data;
        return T();
    }
